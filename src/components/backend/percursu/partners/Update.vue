...
export default {
  mixins: [flashAlert, multFormData, getChargesDatas, getPartnersDatas],
  name: "",
  components: {
    FileUpload
  },
  ...
  methods: {
    ...
    getSinglePartiner: function(username) {
      if (this.partners.length > 0) {
        this.formData = this.partners.find(
    ...
    addCharge() {
      axios
        .post("/charges", this.$data.formData.charge)
        .then(response => {
